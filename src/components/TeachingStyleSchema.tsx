...
import type { TeachingStyleAnalysis } from "@/types";
...
interface TeachingStyleSchemaProps {
  analysis: TeachingStyleAnalysis;
  teacherName: string;
}

export default function TeachingStyleSchema({
  analysis,
  teacherName,
}: TeachingStyleSchemaProps) {
      ...
      <div className="border-b border-stone-100 px-5 py-3">
        <Tabs
          tabs={[
            ...
            { id: "matches", label: "Uyumlu öğrenciler" },
            { id: "guide", label: "Eşleştirme" },
          ]}
          active={tab}
          onChange={setTab}
        />
      ...
      <div className="p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <p className="mb-3 text-xs font-semibold text-green-700">
                {firstName} şu öğrenci tipleriyle iyi eşleşir
              </p>
              <ul className="space-y-2">
                {analysis.excelsWith.map((item) => (
                  <li
                    key={item.type}
            ...
            <div>
              <p className="mb-3 text-xs font-semibold text-red-700">
                Dikkatli eşleştirme gerektiren profiller
              </p>
              <ul className="space-y-2">
                {analysis.lessSuitedFor.map((item) => (
                  <li
                    key={item.type}
        ...
        {tab === "guide" && (
          <div className="space-y-3">
            {analysis.matchingGuide.map((step, i) => (
              <div
                key={step.when}
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-stone-800">{step.recommend}</p>
